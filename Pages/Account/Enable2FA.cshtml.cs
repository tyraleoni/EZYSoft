...
if (!requiredSetup)
 ...
 if (requiredSetup)
 {
 // Sign in the user now that2FA is enabled
 await _signInManager.SignInAsync(user, isPersistent: false);
 // Deactivate other active sessions to enforce single session
 var others = _db.UserSessions.Where(x => x.UserId == user.Id && x.IsActive).ToList();
 ...
 var sess = new UserSession { UserId = user.Id, SessionKey = sessionKey, CreatedAt = DateTime.UtcNow, ExpiresAt = DateTime.UtcNow.AddMinutes(1), IpAddress = ip, UserAgent = ua, IsActive = true };
